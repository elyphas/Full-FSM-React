...
class BootstrapStyles(implicit r: mutable.Register) extends StyleSheet.Inline()(r) {
  ...
  val marginRight = styleWrap("margin-right")

  val paddingRight = styleWrap("padding-right")

  val labelAsBadge = style(addClassName("label-as-badge"), borderRadius(1.em))



  //val navbar = styleWrap("nav", "navbar-nav") //original
  val aside = styleWrap("aside", "")
  val navbar = styleWrap("nav", "navbar-nav navbar-toggleable-md navbar-light bg-faded")
