...
object cmpLstLicitantes {
  // shorthand for styles
  @inline private def bss = GlobalStyles.bootstrapStyles

  case class ItemsFoundProps (items: Seq[Licitante], selectItem: Licitante => Callback)

  private val LstLicitantes = ScalaComponent.builder[ItemsFoundProps]("cmpLstLicitantes")
    .render_P( p => {
      val style = bss.listGroup
      def renderItem(item: Licitante) = {

        val razonSocial = item.razon_social.getOrElse("")
        ...
        <.li(itemStyle, ^.border := "1px solid", /*^.marginTop := 3,*/ ^.borderColor := "#20B2AA",
          ...
          <.div(^.clear:="both", ^.float:="left", ^.padding := "10", ^.border := "1px solid", ^.marginTop := "3",
            ...
            <.span(razonSocial, ^.fontSize := "10")
          ),
          //Button(Button.Props(p.selectItem(item), addStyles = Seq(bss.pullRight, bss.buttonXS)), "Seleccionar"),
          Button(Button.Props(p.selectItem(item), addStyles = Seq(bss.pullRight, bss.buttonXS)), "Eliminar")
        )
      }
  ...
  def apply(items: Seq[Licitante], selectItem: Licitante => Callback) =
          LstLicitantes(ItemsFoundProps(items, selectItem))
}
