...
class CDetPed @Inject()( @NamedDatabase("sicap") protected val dbConfigProvider: DatabaseConfigProvider )
  ...
  private val qryDetPed = TableQuery[tbldetpe]
	...
	private class tbldetpe(tag: Tag) extends Table[DetPedidoAlmacen](tag, Some("almacen"), "detalles_pedidos_aprobados") {
		...
		def * = (noPedido,compra,ejercicio,partida,descripcion_partida,cve_articulo,descripcion_articulo,unidad,cantidad,iva,precio,marcas,anexo,subtotal)<> ( DetPedidoAlmacen.tupled, DetPedidoAlmacen.unapply)
	}

	def getData(ped: EnviarAlmacen): Future[Seq[DetPedidoAlmacen]] = {
		val Aprov = qryDetPed.filter{p => p.noPedido === ped.no_pedido && p.ejercicio === ped.ejercicio && p.compra === ped.compra}
		db.run(Aprov.result)
	}
