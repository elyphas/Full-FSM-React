...
class CPedidoEnviarAlmacen @Inject()(@NamedDatabase("sicap") protected val dbConfigProvider: DatabaseConfigProvider) extends HasDatabaseConfigProvider[JdbcProfile] {

	import profile.api._

	private val tblPedidos = TableQuery[tblPedido]

	def getData(id: EnviarAlmacen ): Future[Seq[PedidoAlmacen]] = {
		val Aprov = tblPedidos.filter{p => p.noPedido === id.no_pedido && p.ejercicio === id.ejercicio && p.compra === id.compra}
		db.run(Aprov.result)
	}

	private class tblPedido(tag: Tag) extends Table[PedidoAlmacen](tag, Some("almacen"), "datosgrales_aprobados"  ) {

		def noPedido 							= column[String]("no_pedido")
		...
		def * = (noPedido, compra, fecha_pedido, tipo_compra, cve_depto, destino, cve_provedor, razon_social, programa,descripcion_programa, fuente_financiamiento, descripcion_fuente, ejercicio )<> ( PedidoAlmacen.tupled, PedidoAlmacen.unapply)
